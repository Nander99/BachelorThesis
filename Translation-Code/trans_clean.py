...
def clean_data(data_list, tags_list, occ):
    # Remove special characters from words within lists
    cleaned_lists = [[re.sub(r'[^\w\s]', '', word1), pos_tag, re.sub(r'[^\w\s]', '', word2), id] for [word1, pos_tag, word2, id] in data_list]

    # Filter out lists where the last word is in stop words, word2 is "unresolved" or contains "_" (and list_item[1] in tags_list)
    ...
    filtered_list = [list_item for list_item in cleaned_lists if list_item[-1] not in stop_words and list_item[0] and list_item[-1] not in useless_words and "_" not in list_item[-1] and list_item[1] in tags_list]
    
    # Count occurrences of each word
    word_counts = Counter([word for pair in filtered_list for word in pair])
    
    # Filter out words appearing less than 10 times
    filtered_words = {word for word, count in word_counts.items() if count >= occ}

    # Filter pairs based on word frequency
    pair_lists = [[word1, word2] for [word1, pos_tag, word2, id] in filtered_list if word1 in filtered_words and word2 in filtered_words]
    print('Amount before cleaning', len(cleaned_lists))
    print('Amount after cleaning', len(pair_lists))

    return array(pair_lists)
...
# All usefull tags ("num", "neorg", "res", "prn", "foreign", "neother", "con", "art", "adj", "hs", "neloc", "int", "adv", "adp", "vrn", "neper", "vrb", "nou")
...
translation_data = clean_data(combine_neloc_data, tags_list, 0)
save_clean_data(translation_data, 'Translation-Pickle-Files/old-new.pkl')
